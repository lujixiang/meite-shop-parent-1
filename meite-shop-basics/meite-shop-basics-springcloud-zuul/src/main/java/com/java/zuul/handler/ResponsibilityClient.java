package com.java.zuul.handler;

import com.java.core.utils.SpringContextUtil;
import com.java.zuul.dao.GatewayHandlerDao;
...
public class ResponsibilityClient {
	@Autowired
	private GatewayHandlerDao gatewayHandlerDao;
	...
	public GatewayHandler getHandler() {
		...
		// 1.数据库查询第一个Handler
		GatewayHandlerEntity firstGatewayHandlerEntity = gatewayHandlerDao.getFirstGatewayHandler();
		...
		while (!StringUtils.isEmpty(nextHandlerId)) {
			// 3.查询数据库下一个Handler信息
			GatewayHandlerEntity nextGatewayHandlerEntity = gatewayHandlerDao.getByHandler(nextHandlerId);
